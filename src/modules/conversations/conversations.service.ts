...
export class ConversationsService {
  ...
  async createPrivate(
    ...
    return await this.create({ type: CONVERSATION_TYPES.PRIVATE, name }, [
      user1,
      user2,
    ]);
  }
  ...
  async startConversation(
    ...
    const conversations = await this.getUsersConversation(
      requestedUser,
      targetUser,
    );
    ...
    if (conversations && conversations.length) {
      ...
      const conversation = conversations.find(
        (item) => item.users.length === numberOfUsers,
      );
    ...
    if (!conversationId) {
      const result = await this.createPrivate(
        targetUser,
        requestedUserRecord,
        targetUserRecord,
      );
